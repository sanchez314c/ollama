...
func (m Layer) Forward(h *mlx.Tensor, cache cache.Cache, B, L int, opts Options) *mlx.Tensor {
	r := h
...
type MultiLinear struct {
	Weight mlx.Tensor `weight:"weight"`
}

func (m MultiLinear) Forward(x *mlx.Tensor) *mlx.Tensor {
...
func (m *Attention) AfterLoad(root *model.Root) ([]*mlx.Tensor, error) {
...
type MLP interface {
	Forward(*mlx.Tensor, int, int, Options) *mlx.Tensor
}
...
func (m dense) Forward(h *mlx.Tensor, _, _ int, opts Options) *mlx.Tensor {
...
type Gate struct {
	...
	CorrectionBias mlx.Tensor `weight:"gate.e_score_correction_bias"`
}

func (m Gate) Forward(h *mlx.Tensor, opts Options) (indices, scores *mlx.Tensor) {
...
func (m *sparse) AfterLoad(*model.Root) ([]*mlx.Tensor, error) {
	w1 := make([]*mlx.Tensor, len(m.Experts))
	w2 := make([]*mlx.Tensor, len(m.Experts))
	w3 := make([]*mlx.Tensor, len(m.Experts))
...
func (m sparse) Forward(h *mlx.Tensor, B, L int, opts Options) *mlx.Tensor {
